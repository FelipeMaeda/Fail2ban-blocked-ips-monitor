...
if [ -e /tmp/ips_atuais.txt ]; then
	...
	if [ $count -eq 0 ]; then
		...
		iptables -S | egrep "DROP|REJECT" | awk '{ print $4 }' >> /tmp/ips_atuais.txt;
	fi;
else
...
count_ips=$(iptables -S | egrep "DROP|REJECT" | awk '{ print $4 }' | egrep -v "$(cat /tmp/ips_atuais.txt | paste -sd'|')" | wc -l)
...
if [ $count_ips -eq 0 ]; then
	...
	iptables -S | egrep "DROP|REJECT" | awk '{ print $4 }' | egrep -v "$(cat /tmp/ips_atuais.txt | paste -sd'|')" | tee -a /tmp/ips_atuais.txt >> /tmp/sendmail.txt;
	echo -e "\nAtenciosamente,\n$SIGNATURE" >> /tmp/sendmail.txt;
fi;
